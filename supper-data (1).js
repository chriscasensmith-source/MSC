...
// '' = same folder as index.html (flat). 'photos/' = inside a photos/ subfolder.
const PHOTO_PREFIX = '';
...
function loadRows() {
  ...
  const out = rows.map((r, i) => {
    ...
    return {
      ...
      // Photo path. If you have a `photos/` subfolder, set PHOTO_PREFIX = 'photos/'.
      // If JPGs live next to index.html (e.g. GitHub upload flattened them), use ''.
      image: o['Image File Name'] ? `${PHOTO_PREFIX}${o['Image File Name']}` : null,
      ratings,
      avg,
